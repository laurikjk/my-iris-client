...
test.describe("Note draft persistence", () => {
  test("should persist draft content between page reloads", async ({page}) => {
    ...
    await page.getByPlaceholder("What's on your mind?").fill(testContent)
    ...
    await expect(page.getByPlaceholder("What's on your mind?")).toHaveValue(testContent)
  })
  ...
  test("should clear draft after publishing", async ({page}) => {
    ...
    await expect(page.getByPlaceholder("What's on your mind?")).not.toBeVisible()
