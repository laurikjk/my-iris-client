...
test.describe('Home Feed Scroll Behavior', () => {
  ...
  test('Mobile: header hides on scroll down and shows on scroll up', async ({ page }) => {
    ...
    const hiddenTransform = await header.evaluate(el => 
      window.getComputedStyle(el).transform
    );
    // Extract Y translation from matrix
    const hiddenY = hiddenTransform.match(/matrix\([^,]+,[^,]+,[^,]+,[^,]+,[^,]+,\s*(-?\d+)/)?.[1];
    ...
    const visibleTransform = await header.evaluate(el => 
      window.getComputedStyle(el).transform
    );
  ...
  test('Desktop two-column: middle column scrolls properly', async ({ page }) => {
    ...
    const middleColumn = await page.locator('[data-main-scroll-container="middle-column"]');
    ...
    if (hasMiddleColumn) {
      ...
      await middleColumn.evaluate(el => { el.scrollTop = 200; });
      await page.waitForTimeout(100);
