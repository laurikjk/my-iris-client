...
export type MessageEntity = MessageType & {session_id: string} // Note: session_id now stores public keys
...
export async function deleteBySession(publicKey: string): Promise<void> {
  await db.messages.where("session_id").equals(publicKey).delete()
}
...
export async function deleteMessage(publicKey: string, messageId: string): Promise<void> {
  await db.messages
    .where("session_id")
    .equals(publicKey)
