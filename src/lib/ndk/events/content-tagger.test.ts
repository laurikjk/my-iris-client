...
describe("await generateContentTags", () => {
  ...
  describe("ContentTaggingOptions", () => {
    it("skips content tagging when skipContentTagging is true", async () => {
      const content =
        "Hello @npub1l2vyh47mk2p0qlsku7hg0vn29faehy9hy34ygaclpn66ukqp3afqutajft #tag1"
    ...
    it("always uses q-tag for note references", async () => {
      const content =
        "Check this out: nostr:note1d2mheza0d5z5yycucu94nw37e6gyl4vwl6gavyku86rshkhexq6q30s0g7"
    ...
    it("copies p-tags from target event when copyPTagsFromTarget is true", async () => {
      ...
      const {tags: processedTags} = await generateContentTags(
        content,
        [],
        opts,
        mockEvent
