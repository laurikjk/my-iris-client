...
describe("NDKEvent encryption (Nip44 & Nip59)", () => {
  ...
  it("encrypts and decrypts an NDKEvent using Nip44", async () => {
    ...
    // Create event
    const sendEvent = await fixture.eventFactory.createSignedTextNote(
      "Test content",
      "alice"
    )
  ...
  it("encrypts and decrypts an NDKEvent using Nip17", async () => {
    ...
    // Create a text note
    const message = await fixture.eventFactory.createSignedTextNote(
      "Hello Nip17!",
      "alice"
    )
    ...
    const decrypted = await giftWrappingModule.giftUnwrap(
      encrypted,
      sendUser,
      receiveSigner
    )
  ...
  it("gift wraps and unwraps an NDKEvent using a private key signer according to Nip59", async () => {
    ...
    const unwrapped = await giftWrappingModule.giftUnwrap(
      wrapped,
      sendUser,
      receiveSigner
    )
  ...
  it("gift wraps and unwraps an NDKEvent using a Nip07 signer for receiving according to Nip59", async () => {
    ...
    const wrapped = await giftWrappingModule.giftWrap(
      message,
      receiveUser,
      receive07Signer
    )
