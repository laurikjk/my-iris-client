...
import type {
  NDKCacheAdapterSqliteWasmOptions,
  SQLDatabase,
  WorkerMessage,
  WorkerResponse,
} from "./types"
  ...
  private async initializeWorker(): Promise<void> {
    ...
    if (!effectiveWorkerUrl) {
      try {
        ...
        console.error(
          "Failed to determine worker URL automatically. Please provide 'workerUrl' option.",
          e
        )
    ...
    try {
      ...
      if (
        ...
        console.error(
          `[NDK-cache-sqlite-wasm] Error while creating worker at "${effectiveWorkerUrl}":`,
          err
        )
      }
      throw err
    ...
    this.worker.onerror = (event: ErrorEvent) => {
      ...
      this.pendingRequests.forEach((p) =>
        p.reject(
          new Error(`Worker failed: ${errorMsg}. Check console for setup instructions.`)
        )
      )
