import type {Hexpubkey, NDKUserProfile} from "@/lib/ndk"
...
export async function getProfiles(
  ...
  if (this.useWorker) {
    if (typeof filter === "function") {
      throw new Error(
        "In worker mode, getProfiles only supports filter descriptors, not functions."
      )
    }
    // Send message to worker and get result
    const result: Array<{pubkey: Hexpubkey; profile: NDKUserProfile}> =
