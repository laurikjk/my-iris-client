...
describe("setEvent", () => {
  ...
  it("should index single-letter tags", async () => {
    ...
    const result = cache.db!.exec("SELECT * FROM event_tags WHERE event_id = ?", [
      event.id,
    ])
  ...
  it("should save profile from kind:0 events", async () => {
    ...
    const result = cache.db!.exec("SELECT * FROM profiles WHERE pubkey = ?", [
      event.pubkey,
    ])
    expect(result[0].values).toHaveLength(1)
  ...
  it("should store relay provenance", async () => {
    ...
    const result = cache.db!.exec("SELECT * FROM event_relays WHERE event_id = ?", [
      event.id,
    ])
    expect(result[0].values).toHaveLength(1)
