...
export function GeohashLocation({event, className = ""}: GeohashLocationProps) {
  ...
  const uniqueGeohashes = [...new Set(geohashTags.map(tag => tag[1]))].sort((a, b) => b.length - a.length)
  ...
  return (
    <div className={`flex items-center gap-1 text-xs text-base-content/50 px-4 -mt-1 mb-2 flex-wrap ${className}`}>
