...
describe("SessionManager", () => {
  ...
  it("should start listening and queue message when no active session exists", async () => {
    ...
    manager.sendText("recipient", "hello")
  ...
  it("should propagate incoming session events to listeners", () => {
    ...
    // Manually setup the session onEvent callback like SessionManager would do
    session.onEvent((event) => {
      ;(manager as any).internalSubscriptions.forEach((callback: any) => 
        callback(event, recipient)
      )
    })
